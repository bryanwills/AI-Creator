import os
from agents.storyboard_generator import NextShotDesigner
...
# script_path = r"examples_for_test\storyboard_generator\input\0-套路.txt"
# working_dir = r"examples_for_test\storyboard_generator\output\0-套路"

# script_path = r"examples_for_test\storyboard_generator\input\3-无间道.txt"
# working_dir = r"examples_for_test\storyboard_generator\output\3-无间道"

script_path = r"examples_for_test\storyboard_generator\input\4-今天抽华子.txt"
working_dir = r"examples_for_test\storyboard_generator\output\4-今天抽华子"
...
storyboard_generator = NextShotDesigner(
    api_key="sk-RsgJVQohu9e1HBMgdYsy9mQFKs3ue4fZXL2iGMjiiupiViQB",
    base_url=r"https://yunwu.ai/v1",
    chat_model="gpt-5-2025-08-07",
)
...
shot_descriptions = []
if os.path.exists(os.path.join(working_dir, "shot_descriptions.json")):
    with open(os.path.join(working_dir, "shot_descriptions.json"), "r", encoding="utf-8") as f:
...
while True:
    with Timer() as timer:
        next_shot_descs = asyncio.run(
            storyboard_generator.design_next_multiple_shots(
                script=script,
        ...
        with open(os.path.join(working_dir, "shot_descriptions.json"), "w", encoding="utf-8") as f:
