...
Web UI 与 TUI 共用同一套 ViMax agent runtime、session、tools 和 `configs/agent.local.yaml` 配置。运行 Web UI 需要 Node.js 18 或更高版本。

在 ViMax 根目录中，首次使用时安装前端依赖，然后启动本地服务：
...
在浏览器中打开 [http://127.0.0.1:4173](http://127.0.0.1:4173)。你可以在 Web UI 中创建并命名项目、与 ViMax Agent 对话、使用斜杠命令、查看生成产物与视频，并修改模型供应商配置。
...
如果提示端口 `4173` 已被占用，Web UI 可能已经启动；也可以改用其他端口：
